...
def read_data_bc(csv_file):
    ...
    with open(csv_file, 'r') as f:
        ...
        for line_num, row in enumerate(reader, start=1):
            ...
            y.append(float(row[1])*1000)
            z.append(float(row[2])*1000)
            ux.append(float(row[3]))

    return np.array(x).reshape(-1,1), np.array(y).reshape(-1,1),np.array(z).reshape(-1,1),np.array(ux).reshape(-1,1)


# eqn_file = (f'../DATA_Veinous_Sinus/p6/velp_step1.csv')
# x_eqn, y_eqn, z_eqn, u_gt, v_gt, w_gt = read_data(eqn_file)

x_eqn, y_eqn, z_eqn,in_size = read_data_bc("../DATA_Veinous_Sinus/post_processing/out_area.csv")

t_eqn = 0.0
t_eqn = np.tile(t_eqn, [x_eqn.shape[0], 1])
...
def eval_one_epoch(sess, ops, num_votes=1, topk=1):
    ...
    for time in np.arange(0,0.8,0.001): 
        ...
        result.to_csv(f'transient_results_out/velp_step{i}.csv', header=True, index=False)
        i = i + 1
